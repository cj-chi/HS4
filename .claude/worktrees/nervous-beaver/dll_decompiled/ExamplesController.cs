...
public class ExamplesController : MonoBehaviour
{
	...
	public void Start()
	{
		...
		realTimeLight1.SetActive(value: true);
		realTimeLight2.SetActive(value: false);
		realTimeLight3.SetActive(value: false);
		...
		{
			objectsParams[i].target.SetActive(value: false);
		}
	...
	public void ClickedArrow(bool rightFlag)
	{
		...
		objectsParams[currentTargetIndex].target.SetActive(value: false);
		...
		PrepareCurrentObject();
		objectsParams[currentTargetIndex].target.SetActive(value: true);
	...
	public void Update()
	{
		...
		{
			GameObject obj = objectsParams[currentTargetIndex].target.transform.Find("Amber").gameObject;
			obj.SetActive(!obj.activeSelf);
		}
		if (Input.GetKeyDown(KeyCode.RightArrow))
		{
			ClickedArrow(rightFlag: true);
		}
		else if (Input.GetKeyDown(KeyCode.LeftArrow))
		{
			ClickedArrow(rightFlag: false);
		}
		if (autoRotation)
		{
		...
		{
			hideTime = Time.time + hideTimeDelay;
			InteractiveUI.SetActive(value: true);
		}
		if (Time.time > hideTime)
		{
			InteractiveUI.SetActive(value: false);
		}
	}
	...
	public void SliderChanged(Slider slider)
	{
		...
		{
			mainCamera.GetComponent<UBER_GlobalParams>().fallIntensity = slider.value;
		}
	...
	public void ClickedSkybox1()
	{
		...
		skyboxSphereActive.GetComponentInChildren<Renderer>().sharedMaterial = skyboxSphereMaterialInactive;
		skyboxSphereActive = skyboxSphere1;
		skyboxSphereActive.GetComponentInChildren<Renderer>().sharedMaterial = skyboxSphereMaterialActive;
		RenderSettings.customReflection = reflectionCubemap1;
		RenderSettings.reflectionIntensity = exposure1;
		RenderSettings.skybox = skyboxMaterial1;
		realTimeLight1.SetActive(value: true);
	...
	public void ClickedSkybox2()
	{
		...
		realTimeLight1.SetActive(value: false);
		realTimeLight2.SetActive(value: true);
		realTimeLight3.SetActive(value: false);
		DynamicGI.UpdateEnvironment();
	}
	...
	public void ClickedSkybox3()
	{
		...
		realTimeLight1.SetActive(value: false);
		realTimeLight2.SetActive(value: false);
		realTimeLight3.SetActive(value: true);
		DynamicGI.UpdateEnvironment();
	}
	...
	public void TogglePostFX()
	{
		...
		mainCamera.gameObject.GetComponent<Bloom>().enabled = component.enabled;
	}
	...
	public void SetTemperatureSun()
	{
		...
		mainCamera.GetComponent<UBER_GlobalParams>().temperature = 20f;
	}
	...
	private void PrepareCurrentObject()
	{
		...
		bool flag = objectsParams[currentTargetIndex].materialProperty == null || objectsParams[currentTargetIndex].materialProperty == string.Empty;
		if (flag)
		{
			materialSlider.gameObject.SetActive(value: false);
		}
		else
		{
			materialSlider.gameObject.SetActive(value: true);
			...
			if (objectsParams[currentTargetIndex].materialProperty == "fallIntensity")
			{
				...
				buttonSun.gameObject.SetActive(value: true);
				buttonFrost.gameObject.SetActive(value: true);
			}
			else
			{
				mainCamera.GetComponent<UBER_GlobalParams>().UseParticleSystem = false;
				buttonSun.gameObject.SetActive(value: false);
				buttonFrost.gameObject.SetActive(value: false);
		...
		anchoredPosition.y = (float)(flag ? 50 : 110) + descriptionTextArea.preferredHeight;
