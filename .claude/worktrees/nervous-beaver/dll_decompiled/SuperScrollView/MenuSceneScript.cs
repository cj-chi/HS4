...
internal class MenuSceneScript : MonoBehaviour
{
	...
	private SceneNameInfo[] mSceneNameArray = new SceneNameInfo[18]
	{
	...
	private void Start()
	{
		...
		for (int i = 0; i < childCount; i++)
		{
			if (i >= num)
			{
				mButtonPanelTf.GetChild(i).gameObject.SetActive(value: false);
				continue;
			}
			mButtonPanelTf.GetChild(i).gameObject.SetActive(value: true);
