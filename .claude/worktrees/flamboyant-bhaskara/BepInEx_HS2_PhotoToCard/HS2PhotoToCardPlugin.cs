...
namespace HS2.PhotoToCard.Plugin
{
    ...
    [BepInDependency("com.hs2.skeletontoggle", BepInDependency.DependencyFlags.SoftDependency)]
    public class HS2PhotoToCardPlugin : BaseUnityPlugin
    {
        ...
        private IEnumerator LoadCardAndScreenshot(string cardPath, string requestFilePath)
        {
            ...
            // --- 1) 乾淨截圖（確保骨骼點關閉）---
            SetBonePointsVisible(false);
            yield return new WaitForEndOfFrame();

            try
            {
                Logger.LogInfo($"[PhotoToCard] CaptureScreenshot (clean): {screenshotPath}");
                UnityEngine.ScreenCapture.CaptureScreenshot(screenshotPath, 1);
                Logger.LogInfo($"[PhotoToCard] Clean screenshot saved: {screenshotPath}");
            }
            catch (Exception ex)
            {
                Logger.LogError($"[PhotoToCard] Clean screenshot failed: {ex.Message}\n{ex.StackTrace}");
            }
            ...
            // --- 2) 骨骼點截圖（開啟骨骼點 GL 渲染）---
            var bonesScreenshotPath = Path.Combine(dir, "game_screenshot_bones.png");
            try { bonesScreenshotPath = Path.GetFullPath(bonesScreenshotPath); } catch { }

            SetBonePointsVisible(true);
            // 等兩幀確保 GL 渲染生效
            yield return new WaitForEndOfFrame();
            yield return new WaitForEndOfFrame();
            ...
            try
            {
                Logger.LogInfo($"[PhotoToCard] CaptureScreenshot (bones): {bonesScreenshotPath}");
                UnityEngine.ScreenCapture.CaptureScreenshot(bonesScreenshotPath, 1);
                Logger.LogInfo($"[PhotoToCard] Bones screenshot saved: {bonesScreenshotPath}");
            }
            catch (Exception ex)
            {
                Logger.LogError($"[PhotoToCard] Bones screenshot failed: {ex.Message}\n{ex.StackTrace}");
            }

            yield return new WaitForEndOfFrame();

            // --- 3) 寫出骨骼 2D 座標 JSON ---
            var chaCtrlForBones = Singleton<CustomBase>.Instance?.chaCtrl;
            if (chaCtrlForBones != null)
                WriteBoneCoordinatesJson(chaCtrlForBones, dir);
        ...
        /// <summary>Key bone names (same as SkeletonCameraHelper.KeyBoneNames).</summary>
        private static readonly HashSet<string> KeyBoneNames = new HashSet<string>
        {
            "cf_J_CheekUp_L", "cf_J_CheekUp_R",
            "cf_J_FaceUp_ty", "cf_J_ChinTip_s",
            "cf_J_Eye_s_L", "cf_J_Eye_s_R",
            "cf_J_Eye01_L", "cf_J_Eye03_L",
            "cf_J_Eye01_R", "cf_J_Eye03_R",
            "cf_J_Mouthup", "cf_J_MouthLow",
            "cf_J_Mouth_L", "cf_J_Mouth_R",
            "cf_J_NoseBridge_t", "cf_J_Nose_tip",
            "cf_J_NoseBase_trs",
            "cf_J_ChinLow",
        };

        /// <summary>Set SkeletonToggleCore.BonePointsVisible via try/catch (soft dependency).</summary>
        private void SetBonePointsVisible(bool visible)
        {
            try
            {
                HS2SkeletonToggle.SkeletonToggleCore.BonePointsVisible = visible;
            }
            catch
            {
                // SkeletonToggle plugin not loaded; ignore
            }
        }

        /// <summary>Write all head bone 2D screen coordinates to game_bone_coords.json.</summary>
        private void WriteBoneCoordinatesJson(ChaControl chaCtrl, string dir)
        {
            if (chaCtrl?.objHeadBone == null) return;
            var cam = Camera.main;
            if (cam == null) return;

            var jsonPath = Path.Combine(dir, "game_bone_coords.json");
            try { jsonPath = Path.GetFullPath(jsonPath); } catch { }

            var bones = new List<(string name, float x, float y, bool isKey)>();
            CollectBoneScreenCoords(chaCtrl.objHeadBone.transform, cam, bones);
            ...
            {
                var (name, x, y, isKey) = bones[i];
                if (i > 0) sb.Append(",");
                sb.Append("{\"name\":\"").Append(name).Append("\",");
                sb.Append("\"x\":").Append(x.ToString("F1")).Append(",");
                sb.Append("\"y\":").Append(y.ToString("F1")).Append(",");
                sb.Append("\"is_key\":").Append(isKey ? "true" : "false").Append("}");
            }
            sb.Append("],\"screen_width\":").Append(screenW);
            sb.Append(",\"screen_height\":").Append(screenH).Append("}");

            try
            {
                File.WriteAllText(jsonPath, sb.ToString());
                Logger.LogInfo($"[PhotoToCard] Bone coords JSON saved: {jsonPath} ({bones.Count} bones)");
            }
            catch (Exception ex)
            {
                Logger.LogError($"[PhotoToCard] Failed to write bone coords JSON: {ex.Message}");
            }
        }

        /// <summary>Recursively collect all bone screen coordinates (depth-first, deterministic order).</summary>
        private static void CollectBoneScreenCoords(Transform t, Camera cam, List<(string name, float x, float y, bool isKey)> outList)
        {
            if (t == null) return;
            Vector3 screen = cam.WorldToScreenPoint(t.position);
            if (screen.z > 0f)
            {
                // Unity screen Y=0 at bottom → flip to image Y=0 at top
                float imageY = UnityEngine.Screen.height - screen.y;
                outList.Add((t.name, screen.x, imageY, KeyBoneNames.Contains(t.name)));
            }
            for (int i = 0; i < t.childCount; i++)
                CollectBoneScreenCoords(t.GetChild(i), cam, outList);
        }
