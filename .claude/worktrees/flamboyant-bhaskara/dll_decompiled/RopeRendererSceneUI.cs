...
public class RopeRendererSceneUI : MonoBehaviour
{
	...
	{
		StopAllCoroutines();
		StartCoroutine(SmoothSetRope(0.3f, 8, 1, 5f, 1, 0.75f, 0f));
	}
	...
	{
		StopAllCoroutines();
		StartCoroutine(SmoothSetRope(0.3f, 6, 1, 5f, 3, 0.5f, 50f));
	}

	public void FadeToStarRope()
	{
		StopAllCoroutines();
		StartCoroutine(SmoothSetRope(0.5f, 5, 2, 2.5f, 1, 0f, 15f));
	}

	public void FadeToFlowerRope()
	{
		StopAllCoroutines();
		StartCoroutine(SmoothSetRope(0.5f, 7, 10, 2f, 1, 0f, 15f));
	}

	public void FadeToSmallCable()
	{
		StopAllCoroutines();
		StartCoroutine(SmoothSetRope(0.15f, 8, 1, 5f, 4, 0.5f, 50f));
	}
	...
	private IEnumerator SmoothSetRope(float _radius, int _edgeCount, int _edgeDetail, float _edgeIndent, int _strandCount, float _strandOffset, float _strandTwist)
	{
		...
		while (!hasFinished)
		{
			...
			if (radiusSlider.value == _radius && edgeDetailSlider.value == (float)_edgeDetail && edgeCountSlider.value == (float)_edgeCount && edgeIndentSlider.value == _edgeIndent && strandCountSlider.value == (float)_strandCount && strandOffsetSlider.value == _strandOffset && strandTwistSlider.value == _strandTwist)
			{
				hasFinished = true;
			}
			yield return 0;
