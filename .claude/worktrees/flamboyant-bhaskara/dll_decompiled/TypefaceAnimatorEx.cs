...
public class TypefaceAnimatorEx : BaseMeshEffect
{
	...
	private Coroutine StartCoroutineEx(IEnumerator routine)
	{
		...
		return Singleton<Game>.Instance.StartCoroutine(routine);
	}
	...
	private IEnumerator WaitPlay()
	{
		...
		if (played != null)
		{
			StopCoroutineEx(played);
		}
		if (timeMode == TimeMode.Speed)
		{
			yield return null;
		}
		animationTime = () => timeMode switch
		{
			TimeMode.Time => duration, 
			TimeMode.Speed => (float)characterNumber / speed, 
			_ => 1f, 
		};
		switch (style)
		{
		case Style.Once:
			played = PlayOnceCoroutine();
			break;
		case Style.Loop:
			played = PlayLoopCoroutine();
			break;
		case Style.PingPong:
			played = PlayPingPongCoroutine();
			break;
		}
		StartCoroutineEx(played);
	...
	private IEnumerator PlayOnceCoroutine()
	{
		...
		if (!isNoWait)
		{
			while (progress < 1f)
			{
				progress += ((timeScale == TimeScale.DeltaTime) ? Time.deltaTime : Time.unscaledDeltaTime) / animationTime();
				yield return null;
			}
		}
		m_isPlaying = false;
	...
	private IEnumerator PlayPingPongCoroutine()
	{
		...
		while (true)
		{
			float num = ((timeScale == TimeScale.DeltaTime) ? Time.deltaTime : Time.unscaledDeltaTime) / animationTime();
			if (isPositive)
			{
				progress += num;
				if (progress > 1f)
