...
public class STRMainMenu : MonoBehaviour
{
	...
	[SerializeField]
	private PointerEnterExitAction pointerEnterExitSelectPlan;

	[SerializeField]
	private GameObject objNoCharacter;
	...
	private IEnumerator Start()
	{
		...
		if (GameSystem.isAdd50)
		{
			...
			itemUIs[0].tgl.interactable = flag;
			if (!flag)
			{
				pointerEnterExitSelectPlan.listActionEnter.Clear();
				pointerEnterExitSelectPlan.listActionEnter.Add(delegate
				{
					objNoCharacter.SetActiveIfDifferent(active: true);
				});
				pointerEnterExitSelectPlan.listActionExit.Clear();
				pointerEnterExitSelectPlan.listActionExit.Add(delegate
				{
					objNoCharacter.SetActiveIfDifferent(active: false);
				});
			}
		}
		objNoCharacter.SetActiveIfDifferent(active: false);
	...
	private IEnumerator CustomCoroutine()
	{
		...
		SpecialTreatmentRoomScene.startCanvas = 1;
