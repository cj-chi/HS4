...
[Serializable]
public class STRCoordinateFileScrollController1
{
	public class RowData
	{
		...
		public STRCoordinateFileInfoComponent1.RowInfo row;
	...
	public Action<STRCoordinateFileInfo1> onSelect;

	public Action<STRCoordinateFileInfo1> onDeSelect;

	private bool isAllnteractable = true;

	public ScrollData selectInfo { get; private set; }

	public void Init(List<STRCoordinateFileInfo1> _lst, bool _isAllnteractable = true)
	{
		scrollerDatas = _lst.Select((STRCoordinateFileInfo1 value, int idx) => new ScrollData
		{
			index = idx,
			info = value
		}).ToArray();
	...
	public STRCoordinateFileInfo1 FindInfoByFileName(string _fileName)
	{
	...
	public void SetNowSelectToggle()
	{
		for (int i = 0; i < view.ShownItemCount; i++)
		{
			...
			{
				STRCoordinateFileInfoComponent1 component = shownItemByIndex.GetComponent<STRCoordinateFileInfoComponent1>();
	...
	private bool IsNowSelectInfo(STRCoordinateFileInfo1 _info)
	{
	...
	private LoopListViewItem2 OnUpdate(LoopListView2 _view, int _index)
	{
		...
		STRCoordinateFileInfoComponent1 component = loopListViewItem.GetComponent<STRCoordinateFileInfoComponent1>();
