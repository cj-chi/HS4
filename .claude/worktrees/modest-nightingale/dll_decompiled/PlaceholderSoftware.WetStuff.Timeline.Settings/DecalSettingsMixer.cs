...
public class DecalSettingsMixer : TemplatedTimeline<DecalSettingsDataContainer, DecalSettingsClip, DecalSettingsMixer, DecalSettingsTrack, DecalSettingsDataContainer.DecalSettingsData, WetDecal>.BaseMixer
{
	private class DecalLayerDataMixer : BaseMixer<DecalSettingsDataContainer.DecalLayerData>
	{
		...
		{
			return new DecalSettingsDataContainer.DecalLayerData(_ch1.Result(defaultValue.Channel1), _ch2.Result(defaultValue.Channel2), _ch3.Result(defaultValue.Channel3), _ch4.Result(defaultValue.Channel4));
		}
	}

	private class DecalLayerChannelDataMixer : BaseMixer<DecalSettingsDataContainer.DecalLayerChannelData>
	{
		...
		{
			return new DecalSettingsDataContainer.DecalLayerChannelData(_inputRangeThresholdMixer.Result(defaultValue.InputRangeThreshold), _inputRangeSoftnessMixer.Result(defaultValue.InputRangeSoftness), _outputRangeMixer.Result(defaultValue.OutputRange));
		}
	}
	...
	protected override DecalSettingsDataContainer.DecalSettingsData Mix(Playable playable, FrameData info, WetDecal trackBinding)
	{
		...
		for (int i = 0; i < inputCount; i++)
		{
			...
			DecalSettingsDataContainer behaviour = ((ScriptPlayable<DecalSettingsDataContainer>)playable.GetInput(i)).GetBehaviour();
			_saturation.Mix(inputWeight, behaviour.Data.Saturation);
			_x.Mix(inputWeight, behaviour.Data.XLayer);
			_y.Mix(inputWeight, behaviour.Data.YLayer);
			_z.Mix(inputWeight, behaviour.Data.ZLayer);
		}
		return new DecalSettingsDataContainer.DecalSettingsData(_saturation.Result(base.Default.Saturation), _x.Result(base.Default.XLayer), _y.Result(base.Default.YLayer), _z.Result(base.Default.ZLayer));
	}
}
