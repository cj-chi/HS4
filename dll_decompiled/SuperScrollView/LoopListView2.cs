...
public class LoopListView2 : MonoBehaviour, IBeginDragHandler, IEventSystemHandler, IEndDragHandler, IDragHandler
{
	...
	public bool IsInit => mListViewInited;
	...
	public ItemPrefabConfData GetItemPrefabConfData(string prefabName)
	{
		...
		{
			if (!(mItemPrefabData.mItemPrefab == null) && prefabName == mItemPrefabData.mItemPrefab.name)
			{
	...
	public void InitListView(int itemTotalCount, Func<LoopListView2, int, LoopListViewItem2> onGetItemByIndex, LoopListViewInitParam initParam = null)
	{
		...
		if (!(mScrollRect == null))
		{
			_ = mDistanceForRecycle0;
			_ = mDistanceForNew0;
			_ = mDistanceForRecycle1;
			_ = mDistanceForNew1;
			mCurSnapData.Clear();
	...
	{
		LoopListViewItem2 shownItemByItemIndex = GetShownItemByItemIndex(0);
		if (shownItemByItemIndex == null || !mSupportScrollBar)
		{
			return;
		}
		for (int i = 0; i < loopCount; i++)
		{
			if (mIsVertList)
			{
				SetItemSize(i, shownItemByItemIndex.CachedRectTransform.rect.height, shownItemByItemIndex.Padding);
			}
			else
			{
				SetItemSize(i, shownItemByItemIndex.CachedRectTransform.rect.width, shownItemByItemIndex.Padding);
			}
		}
	}
	...
	private void InitItemPool()
	{
		foreach (ItemPrefabConfData mItemPrefabData in mItemPrefabDataList)
		{
			...
			string key = mItemPrefabData.mItemPrefab.name;
			if (mItemPoolDict.ContainsKey(key))
			{
				continue;
			}
			...
			if (!(component == null))
			{
				...
				mItemPoolDict.Add(key, itemPool);
