...
public class DynamicBone : MonoBehaviour
{
	...
	{
		public NativeArray<ParticleStruct> calcs;

		[ReadOnly]
		public NativeArray<CollisionStruct> colls;
		...
		public void Execute()
		{
			...
			{
				ParticleStruct value = calcs[i];
				ParticleStruct particleStruct = calcs[value.parentIndex];
				float num = ((value.isTransform != 1) ? particleStruct.worldMatrix.MultiplyVector(value.endOffset).magnitude : (particleStruct.transWorldPosition - value.transWorldPosition).magnitude);
				float num2 = Mathf.Lerp(1f, value.stiffness, weight);
				if (num2 > 0f || value.elasticity > 0f)
				{
					...
					Vector3 vector = ((value.isTransform != 1) ? worldMatrix.MultiplyPoint3x4(value.endOffset) : worldMatrix.MultiplyPoint3x4(value.transLocalPosition));
					Vector3 vector2 = vector - value.position;
				...
				Vector3 vector3 = particleStruct.position - value.position;
		...
		private Vector3 CalcCollider(Vector3 _position, float _particleRadius)
		{
			...
			{
				CollisionStruct collisionStruct = colls[i];
				...
				if (num2 <= 0f)
				{
					...
					if (collisionStruct.bound == DynamicBoneColliderBase.Bound.Outside)
					{
						...
						if (sqrMagnitude > 0f && sqrMagnitude < num4)
						{
							...
							_position = vector + vector2 * (num3 / num5);
						}
					}
					else
					{
						...
						Vector3 vector3 = _position - vector;
						float sqrMagnitude2 = vector3.sqrMagnitude;
						if (sqrMagnitude2 > num7)
						{
							float num8 = Mathf.Sqrt(sqrMagnitude2);
							_position = vector + vector3 * (num6 / num8);
						}
					}
					continue;
				}
				Vector3 center = collisionStruct.center;
				...
				center = worldMatrix.MultiplyPoint3x4(center);
				center2 = worldMatrix.MultiplyPoint3x4(center2);
				if (collisionStruct.bound == DynamicBoneColliderBase.Bound.Outside)
				{
					float num9 = num + _particleRadius;
					float num10 = num9 * num9;
					Vector3 vector4 = center2 - center;
					Vector3 vector5 = _position - center;
					...
					if (num11 <= 0f)
					{
						...
						if (sqrMagnitude3 > 0f && sqrMagnitude3 < num10)
						{
							...
							_position = center + vector5 * (num9 / num12);
						}
						continue;
					}
				...
				Vector3 vector6 = center2 - center;
				Vector3 vector7 = _position - center;
				...
				if (num17 <= 0f)
				{
					...
					if (sqrMagnitude7 > num16)
					{
						...
						_position = center + vector7 * (num15 / num18);
					}
					continue;
				}
	...
	{
		NativeArray<ParticleStruct> calcs = new NativeArray<ParticleStruct>(m_Particles.Count, Allocator.TempJob);
		NativeArray<CollisionStruct> colls = new NativeArray<CollisionStruct>(m_Colliders.Count, Allocator.TempJob);
		for (int i = 0; i < m_Particles.Count; i++)
		{
			Particle particle = m_Particles[i];
			calcs[i] = new ParticleStruct
			{
				...
				transWorldPosition = (particle.m_Transform ? particle.m_Transform.position : Vector3.zero),
				transLocalPosition = (particle.m_Transform ? particle.m_Transform.localPosition : Vector3.zero),
				...
				worldMatrix = (particle.m_Transform ? particle.m_Transform.localToWorldMatrix : Matrix4x4.identity)
			};
		}
		for (int j = 0; j < m_Colliders.Count; j++)
		{
