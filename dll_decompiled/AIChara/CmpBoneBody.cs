...
public class CmpBoneBody : CmpBase
{
	...
	{
		DynamicBoneCollider[] array = dbcBust;
		foreach (DynamicBoneCollider dynamicBoneCollider in array)
		{
	...
	{
		DynamicBone_Ver02[] componentsInChildren = GetComponentsInChildren<DynamicBone_Ver02>(includeInactive: true);
		dynamicBonesBustAndHip = new DynamicBone_Ver02[Enum.GetNames(typeof(ChaControlDefine.DynamicBoneKind)).Length];
		...
		foreach (DynamicBone_Ver02 dynamicBone_Ver in array)
		{
			...
			{
				dynamicBonesBustAndHip[0] = dynamicBone_Ver;
			}
			else if (dynamicBone_Ver.Comment == "Mune_R")
			{
				dynamicBonesBustAndHip[1] = dynamicBone_Ver;
			}
			else if (dynamicBone_Ver.Comment == "Siri_L")
			{
				dynamicBonesBustAndHip[2] = dynamicBone_Ver;
			}
			else if (dynamicBone_Ver.Comment == "Siri_R")
			{
				dynamicBonesBustAndHip[3] = dynamicBone_Ver;
			}
		}
	}
	...
	{
		return GetDynamicBoneBustAndHip((int)area);
	}

	public DynamicBone_Ver02 GetDynamicBoneBustAndHip(int area)
	{
		if (area >= dynamicBonesBustAndHip.Length)
		{
			return null;
		}
		return dynamicBonesBustAndHip[area];
	}

	public void ResetDynamicBonesBustAndHip(bool includeInactive = false)
	{
		if (dynamicBonesBustAndHip == null)
		{
			return;
		}
		DynamicBone_Ver02[] array = dynamicBonesBustAndHip;
		foreach (DynamicBone_Ver02 dynamicBone_Ver in array)
		{
	...
	{
		if (dynamicBonesBustAndHip != null && area < dynamicBonesBustAndHip.Length && !(null == dynamicBonesBustAndHip[area]) && dynamicBonesBustAndHip[area].enabled != enable)
		{
			dynamicBonesBustAndHip[area].enabled = enable;
			if (enable)
			{
				dynamicBonesBustAndHip[area].ResetParticlesPosition();
			}
		}
	}
}
