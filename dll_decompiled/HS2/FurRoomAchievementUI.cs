...
public class FurRoomAchievementUI : MonoBehaviour
{
	[SerializeField]
	...
	[SerializeField]
	private FurRoomMenuUI menuUI;

	[SerializeField]
	private FurRoomAchievementItemScrollController scrollCtrl;

	[SerializeField]
	private FurRoomAchievementScrollController scrolExchangeCtrl;

	private readonly string[] strNextCounts = new string[5] { "残り{0}P", "Remaining Point {0}", "Remaining Point {0}", "Remaining Point {0}", "" };
	...
	private IEnumerator Start()
	{
		...
		scrolExchangeCtrl.onSelect = delegate(FurRoomAchievementScrollController.ScrollData _data)
		{
			...
			status.Yes = delegate
			{
				...
				FurRoomAchievementInfoComponent.RowInfo row = _data.rowData.row;
		...
		(from _ in this.UpdateAsObservable()
			...
			where fm.CGAchievement.alpha >= 0.9f
	...
	private IEnumerator Back()
	{
		...
		menuUI.SetDefaultToggleTransition(-1);
	}
