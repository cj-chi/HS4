...
public class MultiPlay_F1M2 : ProcBase
{
	...
	public MultiPlay_F1M2(DeliveryMember _delivery)
		: base(_delivery)
	{
	...
	public override void setAnimationParamater()
	{
		...
		if (chaFemales[0].objBodyBone != null)
		{
		...
		for (int i = 0; i < chaMales.Length; i++)
		{
			...
			{
				chaMales[i].setAnimatorParamFloat("height", animPar.heights[0]);
				chaMales[i].setAnimatorParamFloat("speed", animPar.speed);
				chaMales[i].setAnimatorParamFloat("motion", animPar.m[0]);
				chaMales[i].setAnimatorParamFloat("breast", animPar.breast);
			}
		}
	...
	private void setPlay(string _playAnimation, bool _isFade = true)
	{
		...
		for (int i = 0; i < chaMales.Length; i++)
		{
			if (!(chaMales[i].objTop == null))
			{
				chaMales[i].setPlay(_playAnimation, 0);
				rootmotionOffsetM[i].Set(_playAnimation);
			}
		}
		if (item != null)
		{
		...
		for (int j = 0; j < lstMotionIK.Count; j++)
		{
			lstMotionIK[j].Item3.Calc(_playAnimation);
		}
		if (item != null)
		{
			item.setPlay(_playAnimation);
	...
	private bool StartAibuProc(bool _isReStart, float _wheel)
	{
		if (nextPlay == 0)
		{
			return false;
	...
	private bool FaintnessStartAibuProc(bool _start, float _wheel)
	{
		if (nextPlay == 0)
		{
			return false;
	...
	private bool LoopAibuProc(int _loop, int _state, float _wheel, HScene.AnimationListInfo _infoAnimList)
	{
		...
		else
		{
			...
			if (_state == 0 && chaFemales[0].objBodyBone != null)
			{
				timeChangeMotionDeltaTimes[0] += Time.deltaTime;
				if (timeChangeMotions[0] <= timeChangeMotionDeltaTimes[0] && !enableMotions[0])
				{
					...
					lerpMotions[0] = new Vector2(ctrlFlag.motions[0], num2);
					lerpTimes[0] = Random.Range(ctrlFlag.changeMotionTimeMin, ctrlFlag.changeMotionTimeMax);
				}
			}
			if (_loop == 0)
			{
	...
	private bool StartHoushiProc(int _state, bool _restart, float _wheel)
	{
		if (nextPlay == 0)
		{
			return false;
		...
		else
		{
			...
			if (chaFemales[0].objBodyBone != null)
			{
				...
				if (timeChangeMotions[0] <= timeChangeMotionDeltaTimes[0] && !enableMotions[0])
				{
					...
					lerpMotions[0] = new Vector2(ctrlFlag.motions[0], num);
					lerpTimes[0] = Random.Range(ctrlFlag.changeMotionTimeMin, ctrlFlag.changeMotionTimeMax);
				}
			}
			if (_loop == 0)
			{
				if (ctrlFlag.speed > 1f && ctrlFlag.loopType == 0)
				{
					setPlay((_state == 0) ? "SLoop" : "D_SLoop");
					ctrlFlag.nowSpeedStateFast = false;
					if (chaFemales[0].objBodyBone != null && (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.voice || voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.startVoice))
					{
						Voice.Stop(ctrlFlag.voice.voiceTrs[0]);
					}
					ctrlFlag.voice.dialog = false;
	...
	private bool StartSonyuProc(bool _restart, int _state, int _modeCtrl, float _wheel)
	{
		if (nextPlay == 0)
		{
			return false;
	...
	private bool LoopSonyuProc(int _loop, int _state, float _wheel, int _modeCtrl, HScene.AnimationListInfo _infoAnimList)
	{
		...
		else if (ctrlFlag.click == HSceneFlagCtrl.ClickKind.FinishBefore)
		{
			...
			{
				if (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.voice)
				{
					Voice.Stop(ctrlFlag.voice.voiceTrs[0]);
				}
				else if (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.startVoice)
				{
					Voice.Stop(ctrlFlag.voice.voiceTrs[0]);
				}
			}
		...
		else
		{
			...
			{
				timeChangeMotionDeltaTimes[0] += Time.deltaTime;
				if (timeChangeMotions[0] <= timeChangeMotionDeltaTimes[0] && !enableMotions[0] && _state == 0)
				{
					timeChangeMotions[0] = Random.Range(ctrlFlag.changeAutoMotionTimeMin, ctrlFlag.changeAutoMotionTimeMax);
					timeChangeMotionDeltaTimes[0] = 0f;
					enableMotions[0] = true;
			...
			if (ctrlFlag.isGaugeHit != oldHit && ctrlFlag.isGaugeHit)
			{
				...
				if (!ctrlFlag.nowAnimationInfo.lstSystem.Contains(0) && _modeCtrl != 4 && (_infoAnimList.nShortBreahtPlay == 1 || _infoAnimList.nShortBreahtPlay == 3))
				{
					ctrlFlag.voice.playShorts[0] = 0;
				}
				ctrlFlag.voice.dialog = false;
			...
			if (ctrlFlag.feel_f >= 0.75f)
			{
				...
				oldHit = false;
				feelHit.InitTime();
				if (chaFemales[0].objTop != null && (voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.voice || voice.nowVoices[0].state == HVoiceCtrl.VoiceKind.startVoice))
				{
					Voice.Stop(ctrlFlag.voice.voiceTrs[0]);
				}
			}
